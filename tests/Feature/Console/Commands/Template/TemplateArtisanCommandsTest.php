...
class TemplateArtisanCommandsTest extends TestCase
{
    ...
    public function test_z_template_commands_full_workflow(): void
    {
        ...
        // force update 시에는 현재 DB 버전이 아니라 파일 버전으로 동기화되어야 함
        $expectedVersion = json_decode((string) file_get_contents(base_path("templates/{$identifier}/template.json")), true, 512, JSON_THROW_ON_ERROR)['version'];
        Template::where('identifier', $identifier)->update(['version' => '0.0.0-test']);
