    ...
    private function clearSingleTemplateCache(string $identifier): void
    {
        ...
        if ($templateRecord) {
            ...
            foreach ($layouts as $layout) {
                ...
                Cache::forget("layout.{$identifier}.{$layout->name}.v0");
                $clearedCount += 2;

                if ($cacheVersion !== 0) {
                    Cache::forget("layout.{$identifier}.{$layout->name}.v{$cacheVersion}");
                    $clearedCount++;
                }
            }
        }
    ...
    private function clearAllTemplateCache(): void
    {
        ...
        foreach ($templates as $templateRecord) {
            ...
            foreach ($layouts as $layout) {
                ...
                Cache::forget("layout.{$templateRecord->identifier}.{$layout->name}.v0");
                $clearedCount += 2;

                if ($cacheVersion !== 0) {
                    Cache::forget("layout.{$templateRecord->identifier}.{$layout->name}.v{$cacheVersion}");
                    $clearedCount++;
                }
            }
